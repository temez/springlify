...
import dev.temez.springlify.platform.annotation.VelocityCommand;
...
public class VelocityCommandInitializer implements BeanPostProcessor {
  ...
  @Lazy
  public VelocityCommandInitializer(@NotNull ServerPlatformAdapter serverPlatformAdapter) {
  ...
  private SimpleCommand getCommandExecutor(@NotNull Object bean) throws RuntimeException {
    if (bean.getClass().isAnnotationPresent(VelocityCommand.class)) {
      ...
      throw new RuntimeException(bean.getClass().getSimpleName() + " must implement CommandExecutor");
    }
    if (bean instanceof SimpleCommand
        && !bean.getClass().isAnnotationPresent(VelocityCommand.class)) {
      log.warn("{} implements CommandExecutor but not annotated with @SpigotCommand,"
          + " so it wouldn't be registered", bean.getClass().getSimpleName()
      );
    }
    return null;
  }
  ...
  public Object postProcessAfterInitialization(@NotNull Object bean, @NotNull String beanName)
    ...
    serverPlatformAdapter.registerCommandExecutor(
